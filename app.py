...
with col2:
    ...
    for p in ax2.patches:
        width = p.get_width()
        ax2.text(width + 2, p.get_y() + p.get_height()/2.,
                 '{:1.0f}'.format(width),
                 ha='left', va='center')
    plt.tight_layout()
    st.pyplot(fig2)
...
with col3:
    ...
    for p in ax3.patches:
        width = p.get_width()
        ax3.text(width + 2, p.get_y() + p.get_height()/2.,
                 '{:1.0f}'.format(width),
                 ha='left', va='center')
    plt.tight_layout()
    st.pyplot(fig3)
...
with col4:
    ...
    for p in ax4.patches:
        width = p.get_width()
        ax4.text(width + 2, p.get_y() + p.get_height()/2.,
                 '{:1.0f}'.format(width),
                 ha='left', va='center')
    plt.tight_layout()
    st.pyplot(fig4)

col_left_idade, col_center_idade, col_right_idade = st.columns([1,2,1])

with col_center_idade:
...
st.divider()
col_left_temporal, col_center_temporal, col_right_temporal = st.columns([5,1,1])

with col_left_temporal:
...
st.divider()
col_left_geografica, col_center_geografica, col_right_geografica = st.columns([5,1,1])

with col_left_geografica:
...
col_left_numeroMortes, col_center_numeroMortes, col_right_numeroMortes = st.columns([1,3,1])

with col_center_numeroMortes:
...
st.divider()
st.markdown("<h3>Para rodar este aplicativo Streamlit localmente, navegue até o diretório raiz do projeto no terminal e execute:</h3>", unsafe_allow_html=True)
